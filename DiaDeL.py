...
#Mutational Signature Feedforward Neural Network Architecture
...
signatures_cancerType_df = tcga_signatures_dataframe.join(organs_dataframe)
tcga_clinical_dataframe = pickle.load(open(main_folder + "TCGA_clinical_dataframe.pickle","rb"))
x_dim = 30
...
for cancer_class in classes:
    ...
    while(indices_specific):
        ...
        for shuffled_epoch in range(20):
            ...
            for epoch in range(50):
                for index in range(0, len(training), batch_size):
                    ...
                    batch = signatures_cancerType_df.loc[training[index : index + batch_size]]
                    batch = batch.to_numpy()
                    X = batch[:,0:30].astype(float)
        ...
        for index in range(0, len(test), batch_size):
            ...
            test_list = signatures_cancerType_df.loc[test[index : index + batch_size]]
            test_list = test_list.to_numpy()
            test_list = test_list[:,0:30].astype(float)
...
pickle.dump(dic_cancer_accuracies, open(main_folder + "RESULTS/MutationalSignature/MutationalSignature-Accuracies.pickle","wb"))
